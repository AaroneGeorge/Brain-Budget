...
> ℹ️ Source links throughout this document are repo‑relative with line anchors (e.g. `server/src/buyer.ts#L43-L52`) and resolve to the exact lines on the hosted repository. This is a **deep technical companion** to [`README.md`](README.md); the original is preserved unchanged.
...
9. [Track qualification](#9-track-qualification)
10. [Run it yourself](#10-run-it-yourself)
11. [Scripts reference](#11-scripts-reference)
12. [What we'd build next](#12-what-wed-build-next)
...
### Why this design is the maximally judge‑aligned narrative

The three sponsor technologies compose into one self‑demonstrating loop — *an agent that pays for its own brain with delegated funds* — that exercises **four paid tracks from a single codebase**:

- A user delegates a **scoped budget** to an AI agent → **MetaMask Smart Accounts + ERC‑7710**, with caveats enforced on‑chain.
- The agent autonomously pays for **its own Venice inference per request in USDC** → **x402 + Venice AI**, no API key anywhere.
- Those redemptions are **relayed gaslessly through 1Shot** (gas paid in USDC, status streamed from Ed25519‑verified webhooks) → **1Shot Permissionless Relayer**.
- The orchestrator **redelegates a narrower sub‑budget** to a critic sub‑agent that pays for its own review → **A2A Coordination**, settled on‑chain as a 3‑hop chain.
...
> Flipping `CHAIN=base` ↔ `CHAIN=baseSepolia` switches USDC, the MetaMask facilitator, and the 1Shot relayer endpoint with **no code edits** — the identical agent runs live on Base mainnet, satisfying the mainnet requirement.
...
> `hasBudgetFor` projects spend forward before each paid call and **reserves** the calls needed to finish coherently — the autonomous self‑governance a judge wants to see. (In the live run, this fired the `stopping early: $0.03 of $0.05 spent, reserving the rest for synthesis` event after 2 of 3 research steps.)
...
> A forged webhook is rejected with `401`; only verified callbacks advance the run state. The `GET /state` route ([L27–L45](server/src/index.ts#L27-L45)) reports live balances with `.catch(() => null)` fallbacks (so a transient RPC blip shows `—` instead of crashing the demo) and a `veniceMocked` flag so a judge can see real‑vs‑mock at a glance. Actors are built once and memoized in [`config.ts getActors()` L33–L57](server/src/config.ts#L33-L57); the optional critic exists only when `CRITIC_PRIVATE_KEY` is set.
...
├── README.md                    the original judged artifact
└── README2.md                   ← you are here (deep technical companion)
```
...
### Base Sepolia — the special case (free, and where the "kill shot" is proven)

Venice's x402 endpoint is **mainnet‑only**, so the *real* inference demo must run on Base mainnet. But the **delegation lifecycle and the on‑chain caveat rejection** are proven for free on **Base Sepolia** via [`pnpm demo:e2e`](server/scripts/e2e.ts):
...
| Track | Where to look (one‑line proof) |
|---|---|
| **Best x402 + ERC‑7710** | Every inference call is an x402 payment with `assetTransferMethod: "erc7710"` — buyer [`server/src/buyer.ts`](server/src/buyer.ts), seller [`server/src/gateway.ts`](server/src/gateway.ts), settled by the MetaMask facilitator redeeming the user's caveated delegation. **Not cosmetic: remove the delegation and the agent cannot think.** |
| **Best Agent** | The agent autonomously plans, buys its own inference, tracks its budget, reserves calls, stops when the caveat enforcer says no, delegates review to a sub‑agent, invoices its fee, and reports spend — [`server/src/agent/loop.ts`](server/src/agent/loop.ts). The Smart Accounts Kit is its *only* spending mechanism. |
| **Best use of Venice AI** | 100% of inference is Venice (`llama-3.3-70b`) via Venice's **native x402** with wallet auth — **no API key in the entire stack** ([`server/src/venice.ts`](server/src/venice.ts)). |
| **1Shot Permissionless Relayer** | EIP‑7702 onboarding *and* the completion fee are `redeemDelegations` relayed via `relayer_send7710Transaction` — estimate‑first price lock, gas paid in USDC, **Ed25519‑verified webhook receiver** feeding a live UI tape ([`server/src/relayer.ts`](server/src/relayer.ts)). |
| **A2A Coordination** | The orchestrator redelegates a narrowed sub‑budget to a critic via `createSubDelegation` ([`packages/shared/src/delegation.ts`](packages/shared/src/delegation.ts)); the facilitator redeems the **3‑hop chain** `user → agent → critic` — settled on‑chain (tx #5 above). |
